...
#' par2 <- 4
#' \dontshow{n <- 50}
#' ## A calibration surface depending on two variables
#' eta0 <- 1
...
#' 
#' ## 4-dimensional matrix X of covariates
#' covariates.distr <- mvdc(normalCopula(rho, dim = 4),
#'                                  c("unif"), list(list(min = 0, max = 1)),
#'                                  marginsIdentical = TRUE)
#' X <- rMvdc(n, covariates.distr)
#' colnames(X) <- paste("x",1:4,sep="")
#' 
#' ## U in [0,1]x[0,1] depending on the first two columns of X
#' U <- condBiCopSim(fam, function(x1,x2) {eta0+sum(mapply(function(f,x)
#'   f(x), calib.surf, c(x1,x2)))}, X[,1:2], par2 = 4, return.par = TRUE)
#' 
#' ## Selection using AIC (take about 30sec on single core) 
