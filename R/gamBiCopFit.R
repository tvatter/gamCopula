...
gamBiCopFit <- function(data, formula = ~1, family = 1, tau = TRUE,
  ...
  res <- tryCatch({
    ...
    msg <- paste(
      "A problem occured at the first iteration of the ",
      method, "algorithm:\n"
    )
    msg <- c(msg, paste(err))
  ...
  while ((k < n.iters) &
    ...
    res <- tryCatch({
      ...
      msg <- paste(
        "A problem occured at iteration", k, "of the ",
        method, "algorithm:\n"
      )
      msg <- c(msg, paste(err))
      #       msg <- c(msg, paste("...... switching to",
    ...
    if (is.na(tt$eps)) {
      message(paste(
        "A problem occured at the ", k, "th iteration of the ",
  ...
  out <- list(
    res = gamBiCop(family, mm, par2, tau),
...
valid.gamBiCopFit <- function(data, n.iters, tau, tol.rel, method, verbose,
  ...
  if (is.null(names(data)) || !all(is.element(c("u1", "u2"), names(data)))) {
    warnings(paste0(
      "u1 and u2 not found in data. The first two columns are",
